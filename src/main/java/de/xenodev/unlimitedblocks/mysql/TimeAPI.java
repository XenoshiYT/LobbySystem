...
public class TimeAPI {
    ...
    public boolean playerExists(UUID uuid){
        ...
        try{
            ResultSet rs = LobbySystem.mysql.query("SELECT * FROM Time WHERE UUID= '" + uuid + "'");
            if(rs.next()){
    ...
    public void createPlayer(UUID uuid){
        ...
        if(!(playerExists(uuid))){
            LobbySystem.mysql.update("INSERT INTO Time(UUID, HOURS, MINUTES, SECONDS) VALUES ('" + uuid + "', '0', '0', '0');");
        }
    }
    ...
    public void setHours(UUID uuid, Integer hours){
        if(playerExists(uuid)){
            LobbySystem.mysql.update("UPDATE Time SET HOURS= '" + hours + "' WHERE UUID= '" + uuid + "';");
        }else{
            createPlayer(uuid);
            setHours(uuid, hours);
        }
    ...
    public void setMinutes(UUID uuid, Integer minutes){
        if(playerExists(uuid)){
            LobbySystem.mysql.update("UPDATE Time SET MINUTES= '" + minutes + "' WHERE UUID= '" + uuid + "';");
        }else{
            createPlayer(uuid);
    ...
    public void setSeconds(UUID uuid, Integer seconds){
        if(playerExists(uuid)){
            LobbySystem.mysql.update("UPDATE Time SET SECONDS= '" + seconds + "' WHERE UUID= '" + uuid + "';");
        }else{
            createPlayer(uuid);
